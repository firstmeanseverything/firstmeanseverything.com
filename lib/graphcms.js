...
const getProgramPage = async (args, preview = false) =>
...
const getProgramsList = async (args, preview = false) =>
  await graphCmsClient.request(ProgramsListQuery, {
    stage: preview ? 'DRAFT' : 'PUBLISHED',
    ...args
  })

const getProgramsPaths = async (args, preview = false) =>
  await graphCmsClient.request(ProgramsPathsQuery, {
    stage: preview ? 'DRAFT' : 'PUBLISHED',
    ...args
  })

const getSampleProgramPage = async (args, preview = false) =>
  await graphCmsClient.request(SampleProgramPageQuery, {
    stage: preview ? 'DRAFT' : 'PUBLISHED',
    ...args
  })

const getSampleProgramsList = async (args, preview = false) =>
